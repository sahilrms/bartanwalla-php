	...
	if(isset($_SESSION['user'])){
		...
		try {
			...
			foreach($stmt as $row){
				...
				$images = (!empty($row['photo'])) ? explode(',', $row['photo']) : ['images/noimage.jpg'];
				$imageHTML = '';
				foreach ($images as $img) {
					$imageHTML .= "<img src='images/".trim($img)."' width='30px' height='30px' style='margin-right: 5px;'>";
				}

				$subtotal = $row['price'] * $row['quantity'];
				$total += $subtotal;
				$output .= "
					<tr data-item-id='".$row['cartid']."'>
		...
		if(count($_SESSION['cart']) != 0){
			...
			foreach($_SESSION['cart'] as $row){
				...
				// Handle multiple images
				$images = (!empty($product['photo'])) ? explode(',', $product['photo']) : ['images/noimage.jpg'];
				$imageHTML = '';
				foreach ($images as $img) {
					$imageHTML .= "<img src='images/".trim($img)."' width='30px' height='30px' style='margin-right: 5px;'>";
				}
